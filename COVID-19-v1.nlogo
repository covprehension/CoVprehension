...
to setup_globals
  ;; From interface
  set %Inr 0.0
  set scénario "Densité de population faible"
  set DISTANCIATION? false
  set %RESPECT 0.8
  ;;
  ask patches [set pcolor white]
  if scénario = "Densité de population faible"
  [
    set population-size 100
    set transmission-distance 1
  ]
  if scénario = "Densité de population forte"
  [
    set population-size 1000
    set transmission-distance 0.5
  ]
  set nb-infected-initialisation 1
  set distanciation-distance 3
  set walking-angle 20
  set speed 0.5
end
...
to set_respect_rules
  if %Respect < 100
  [
  ask n-of (population-size - round (%Respect * population-size / 100)) citizens
    [set respect-rules? 1]
  ]
end
...
to spread_virus
  ...
  [
    let others-infected other citizens in-radius transmission-distance with [epidemic-state = 1 or epidemic-state = 2]
    if any? others-infected
    [
      let target one-of others-infected
      ask target [ set nb-other-infected nb-other-infected + 1]

      if random 100 <= proba_transmission
      [
        ifelse %Inr > 0
        [
          ifelse random 100 > %Inr
          [set epidemic-state 1]
...
to show_epidemic_state
  ask citizens
  [
    if epidemic-state = 0  [set color green]
    if epidemic-state = 1 [set color red]
    if epidemic-state = 3 [set color gray]
;    if epidemic-state = 2
;    [ifelse Montre_Inr?
;      [set color orange]
;      [set color green]]
...
"Ir" 1.0 0 -2674135 true "" "if population-size > 0 [plot nb_Ir]"
"Inr" 1.0 0 -817084 true "" "if population-size > 0 [plot nb_Inr]"
"I" 1.0 0 -5825686 true "" "if population-size > 0 [plot nb_I]"
"R" 1.0 0 -7500403 true "" "if population-size > 0 [plot nb_R]"
