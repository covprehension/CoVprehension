...
>  NB: _Il ne faut pas faire de parallèle trop direct entre ce que l'on appelle un jour dans le modèle et les jours tel que nous les connaissons. À l'image d'une journée que nous vivons, un jour dans le modèle correspond à un ensemble d'intéractions que les agents ont à l'extérieur et à l'intérieur de leur foyer. Cela ne va pas plus loin : le nombre d'interactions dans un jour du modèle n'est pas forcéménent réaliste, comme la probabilité de transimisson de la maladie etc. Le but ici est d'explorer l'impact de la temporalité de mise en oeuvre du **confinement ciblé** sur la dynamique de propagation de la maladie.
